...
def main():
    ...
    if not os.path.exists(vehicles_dir):
        ...
        print("Make sure you run generate_seo_pages.py first!")
        return
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        # Add the main vehicle directory index manually if it wasn't caught
        if not any(u[0].endswith("vehicles/index.html") for u in urls):
             f.write('  <url>\n')
             f.write(f'    <loc>{BASE_URL}index.html</loc>\n')
    ...
    print(f"Successfully generated {OUTPUT_FILE} with {len(urls)} URLs.")
